...
Usage:
    ...
    df = store.load("funding_zscore_14d", asset="BTCUSDT", version="v1")
    df = store.query("SELECT asset, date_trunc('day', epoch_us(ts)) AS day, avg(value) FROM features GROUP BY 1, 2")
"""
...
class FeatureStore:
    ...
    def load(
        ...
        Args:
            feature_id: e.g. "funding_zscore_14d"
            asset:      e.g. "BTCUSDT"
            version:    e.g. "v1"
            d_from:     ISO date string inclusive, e.g. "2024-01-01"
            d_to:       ISO date string inclusive, e.g. "2024-03-31"
        """
        glob = str(
        ...
        if d_from:
            ...
            base += f" AND ts >= {us}"
        if d_to:
            us = int((pd.Timestamp(d_to, tz="UTC") + pd.Timedelta(days=1)).timestamp() * 1_000_000)
            base += f" AND ts < {us}"
        base += " ORDER BY ts"
